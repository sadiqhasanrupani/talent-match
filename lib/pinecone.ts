...
try {
    ...
    const textToEmbed = resumeText || candidateData.resumeText || candidateData.skills;
...
try {
    ...
    if (!resumeText) {
    console.warn(`No text extracted from resume for candidate ${candidateId}`);
    return await storeCandidateEmbedding(candidateData);
    }
    
    // Store embedding with the extracted resume text
